...
export default function GatewayComponent() {
  ...
  return (
    <div>
      <h1 className='text-secondary p-6 mb-8 text-center'>Please ensure that payment is made by the specified due date to avoid any disruptions in your service.
        ...
        <div className='rounded-lg w-fit bg-secondary p-6  m-auto justify-center items-center'>
            {invoiceDetails.map((invoice, index) => (
             <div key={index}>
               <h2 className='p-2 text-primary text-2xl font-bold'>Proceed to payment</h2>
               ...
               <p><text className='font-bold p-6'>Payment type</text>{invoice.paymentType}</p>
                <button className='bg-primary rounded-lg p-2 w-96 text-white mt-6 text-center'>Pay Now</button>
               </div>
            ))}
        </div>
    </div>
